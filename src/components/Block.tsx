...
export default function Block({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`group relative flex gap-3 py-7 ${
        isSelected ? 'is-selected' : ''
      } ${isStreaming ? 'is-streaming' : ''}`}
    >
       <div className="w-8 shrink-0 pt-0.5 text-right sm:w-10">
         <span
           className={`text-[11px] font-medium tabular-nums transition-colors ${numberColor}`}
         >
           {String(index + 1).padStart(2, '0')}
         </span>
         <div
           className={`mt-1.5 flex flex-col items-center gap-0.5 z-10 transition-opacity duration-150 ${controlsReveal} sm:absolute sm:right-0 sm:top-7 sm:mt-0 sm:flex-row sm:items-center sm:gap-0`}
         >
           <button
             {...listeners}
             {...attributes}
               ...
               isSelected
                 ...
                 : eligibleForBridge
                   ...
                   : bridgeMode
                     ...
                     : `${controlColor} hover:bg-surface hover:text-ink`
             }`}
           >
             <svg width="16" height="16" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden="true">
       ...
       >
         <textarea
           ...
           rows={1}
           className={`min-h-[60px] w-full resize-none overflow-hidden border-0 bg-transparent p-0 pr-10 sm:pr-[160px] text-[15px] font-normal leading-[1.65] outline-none transition-colors placeholder:text-ink-muted ${
             isActive ? 'text-ink' : 'text-ink-secondary'
           }`}
         />

         {isActive && (
           <div className="mt-1.5 flex animate-toolbar-in flex-wrap items-center gap-1">
             ...
             <button
               ...
               className={`rounded-[3px] px-1.5 py-0.5 text-[11px] transition-colors disabled:opacity-30 ${
                 aiEnabled
